...
#define DRIVER_NAME "simple-keys"
...
static int simple_key_remove(struct platform_device *pdev)
{
	int i;
...
static const struct of_device_id simple_key_of_match[] = {
	{ .compatible = "sample,simple-key", .data = NULL},
	{},
};
MODULE_DEVICE_TABLE(of, simple_key_of_match);
#else
#define simple_key_of_match NULL
#endif

static struct platform_driver simple_key_platform_driver = {
	.probe = simple_key_probe,
	.remove = simple_key_remove,
	.driver = {
		.name = DRIVER_NAME,
		.of_match_table = simple_key_of_match,
	},
};

static int __init simple_key_dev_init(void)
{
	int ret;

	ret = platform_driver_register(&simple_key_platform_driver);
	if (ret) {
...
static void __exit simple_key_dev_exit(void)
{
	platform_driver_unregister(&simple_key_platform_driver);
}

module_init(simple_key_dev_init);
module_exit(simple_key_dev_exit);
