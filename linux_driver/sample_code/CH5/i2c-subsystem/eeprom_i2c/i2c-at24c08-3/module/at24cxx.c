...
#ifdef AT24C08
	#define EEP_ADD_LEN 1
#else
	#define EEP_ADD_LEN 2
#endif
 
int i2c_write_read(const struct i2c_client *client, char *buf)
{
	int ret;
	struct i2c_adapter *adap = client->adapter;
	struct i2c_msg msg[2];
	u8 send_addr[2], recv_data = 0;
	...
	msg[0].len = EEP_ADD_LEN;
	msg[0].buf = (char *)&send_addr[0];

	msg[1].addr = client->addr;
	msg[1].flags = client->flags | I2C_M_RD;
	msg[1].len = 1;
	msg[1].buf = (char *)&recv_data;
...
int write_eep(struct i2c_client *client, u8 *val, loff_t count, loff_t offset)
{
	...
	for (i = 0; i < count; i++) {
		...
		send_data[2] = val[i];
#endif
		ret = i2c_master_send(client, send_data, EEP_ADD_LEN + 1);
		if (ret < 0) {
			dev_err(&client->dev, "%s count %d fail\r", __func__, i);
			return -1;
		}
#ifdef AT24C08
		addr = addr + 1;
...
#else
	send_buf[0] = (addr & 0xFF00) >> 8;
	send_buf[1] = addr & 0xFF;
#endif

	for (i = 0; i < count; i++) {
		i2c_write_read(client, send_buf);
		addr = addr + 1;
#ifdef AT24C08
		val[i] = send_buf[1];
		send_buf[0] = addr;
	...
	size_t count, loff_t *f_pos)
{
	...
	pr_info("%s f_pos %d, count %d\r\n", __func__, (u8)*f_pos, (u32)count);
